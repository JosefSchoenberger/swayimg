...
static void scale_fixed(struct viewport* vp, enum vp_scale scale)
{
    ...
    viewport_scale_abs(vp, factor, (double)vp->width / 2.0,
                       (double)vp->height / 2.0);
}
                        ...
                        double center_y)
{
    ...
    const double img_center_x = (center_x - vp->x) / vp->scale;
    const double img_center_y = (center_y - vp->y) / vp->scale;
    ...
    // restore center
    vp->x = (double)center_x - img_center_x * vp->scale;
    vp->y = (double)center_y - img_center_y * vp->scale;
